...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     /** Private variable used to accept the arguments passed in constructor**/
     vector<Point<Dim>> points;

     /**
      * Helper function used for quickSelect.
      *
      * Group a list into two parts in linear time: a part that is less than a
      * certain element, and a part that is greater than the element.
      *
      * Detailed see: https://en.wikipedia.org/wiki/Quickselect
      */
      ...
      * (i.e. left <= k <= right)
      *
      * The search space within the array is changing for each round - but the
      * list is still the same size. Thus, k does not need to be updated with
      * each round.
      *
      * Detailed see: https://en.wikipedia.org/wiki/Quickselect
      */
     ...
     /**
      * Quicksort function using the partition() and select() helper function.
      */
     KDTreeNode* quickSort(int low, int high, int dimension);

     /**
      * Helper function for destructor, operator = and copy constructor.
      */
      ...
      */
     KDTreeNode* copy(const KDTree<Dim>& other);

     /**
      * Helper function used for findNearestNeighbor.
      */
     Point<Dim> NNHelper(const Point<Dim>& query, int low, int high, int dimension) const;
};

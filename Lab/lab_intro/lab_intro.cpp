...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      double distance_ = std::sqrt((centerX - x) * (centerX - x) + (centerY - y) * (centerY - y));
      if (distance_ <= 160) {
        pixel.l = (1 - 0.5 * distance_ / 100) * pixel.l;
...
PNG illinify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      int hueDistanceO_ = std::min(std::abs(hueP_ - hueOrange_), 360 - std::abs(hueP_ - hueOrange_));
      int hueDistanceB_ = std::min(std::abs(hueP_ - hueBlue_), 360 - std::abs(hueP_ - hueBlue_));

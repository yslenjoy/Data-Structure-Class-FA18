...
class InorderTraversal : public TreeTraversal<T> {
  public: 
	    ...
	    : root(root)
    {
        stack.push(root);	
        // your code here
        typename BinaryTree<T>::Node* leftNode = root->left;
        while (leftNode != NULL) {
            stack.push(leftNode);
    ...
    void add(typename BinaryTree<T>::Node *& treeNode) {
        // your code here
        typename BinaryTree<T>::Node* rightNode = treeNode->right;
        if (rightNode != NULL) {
            stack.push(rightNode);
            typename BinaryTree<T>::Node* rightLeftNode = rightNode->left;
            while (rightLeftNode != NULL) {
                stack.push(rightLeftNode);

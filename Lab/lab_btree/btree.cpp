...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
    if (first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == key && !subroot->elements.empty()) {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    if (!child->is_leaf) {
        ...
        // rather than assign(mid_child_itr + 1, child->children.end())

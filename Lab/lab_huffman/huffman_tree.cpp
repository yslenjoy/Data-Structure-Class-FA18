                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* node1 = singleQueue.front();
    TreeNode* node2 = mergeQueue.front();
    if (node1->freq.getFrequency() < node2->freq.getFrequency()) {
        singleQueue.pop();
        return node1;
    } else {
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned x = 0; x < frequencies.size(); x++) {
        Frequency add = frequencies[x];
        TreeNode* leafNode = new TreeNode(add);
        singleQueue.push(leafNode);
    }
    while (!(singleQueue.empty() && mergeQueue.size() == 1)) {
      TreeNode* merge1 = removeSmallest(singleQueue, mergeQueue);
      TreeNode* merge2 = removeSmallest(singleQueue, mergeQueue);
      if (merge1 == NULL) {
        return;
      }
      if (merge2 == NULL) { // when singleQueue has only one element
        TreeNode* merged = new TreeNode(merge1->freq);
        mergeQueue.push(merged);
        return;
      }
      TreeNode* merged = new TreeNode(merge1->freq.getFrequency() + merge2->freq.getFrequency());
      merged->left = merge1;
      merged->right = merge2;
      mergeQueue.push(merged);
    }
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
     if (current->left == NULL && current->right == NULL) {
       ...
       bfile.writeBit(0);
       this->writeTree(current->left, bfile);
       this->writeTree(current->right, bfile);
       // return;
     }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit() == 1) {
      ...
      TreeNode* node1 = new TreeNode(f);
      head = node1;
    } else {
      TreeNode* node2 = new TreeNode(0);
      node2->left = this->readTree(bfile);
      node2->right = this->readTree(bfile);
      head = node2;
    }
    return head;
}
